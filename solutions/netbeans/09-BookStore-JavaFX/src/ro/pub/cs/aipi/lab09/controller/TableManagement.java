...
public class TableManagement {
    ...
    public void initialize() throws SQLException {
        ...
        for (String attribute : attributes) {
            ...
            if (parentTable != null) {
                ...
                List<List<String>> parentTableContent = databaseOperations.getTableContent(parentTable, null, null, null, null, null);
                ...
                if (currentIndex == 0) {
                    attributeTextField.setText(Integer.toString(databaseOperations.getTablePrimaryKeyMaximumValue(tableName) + 1));
    ...
    public void populateTableView(String whereClause) {
        try {
            List<List<String>> values = DatabaseOperationsImplementation.getInstance().getTableContent(tableName,
                    null,
    ...
    private void tableContentTableViewHandler(MouseEvent mouseEvent) {
        List<String> tableViewRecord = ((Model) tableContentTableView.getSelectionModel().getSelectedItem()).getValues();
        int currentIndex = 0;
        for (String value : tableViewRecord) {
            if (attributesControls.get(currentIndex) instanceof ComboBox) {
                try {
                    String parentTable = databaseOperations.getForeignKeyParentTable(tableName, attributesLabels.get(currentIndex).getText());
                    if (parentTable != null) {
                        List<List<String>> parentTableReferrence = null;
                        parentTableReferrence = databaseOperations.getTableContent(parentTable,
                                null,
                                databaseOperations.getTablePrimaryKey(parentTable) + "=" + tableViewRecord.get(currentIndex),
                                null,
                                null,
                                null);
    ...
    private void insertButtonHandler(MouseEvent mouseEvent) {
        ...
        try {
            databaseOperations.insertValuesIntoTable(tableName,
                    null,
                    values,
                    false);
    ...
    private void deleteButtonHandler(MouseEvent mouseEvent) {
        ...
        try {
            if (attributes != null && values != null && !attributes.isEmpty() && !values.isEmpty()) {
                databaseOperations.deleteRecordsFromTable(tableName,
                        attributes,
                        values,
                        null);
            }
    ...
    private void clearButtonHandler(MouseEvent mouseEvent) {
        ...
        for (Control attributeControl : attributesControls) {
            if (attributeControl instanceof ComboBox) {
                ...
                try {
                    if (currentIndex == 0) {
                        ((TextField) attributeControl).setText(Integer.toString(databaseOperations.getTablePrimaryKeyMaximumValue(tableName) + 1));
                    } else {
